...
[GlobalClass]
[Tool]
public partial class RunStateTable : Resource
{
    [Export] public Dictionary<RunStateType, PackedScene> scenes = new Dictionary<RunStateType, PackedScene>();
    ...
    Callable Scan => Callable.From(() =>
    {
        ...
        for (int i = 0; i < (int)RunStateType.Count; i++)
        {
            RunStateType enumValue = (RunStateType)i;
            string enumValueName = ((RunStateType)i).ToString();
        ...
        for (int i = 0; i < (int)RunStateType.Count; i++)
        {
            RunStateType suitEnum = (RunStateType)i;
            string suitEnumName = suitEnum.ToString();

            if (!ResourceLoader.Exists($"{scriptPath}/{(RunStateType)i}GameState.cs"))
            {
